...
router.post('/', async (req, res) => {
    try {
        ...
        var options = {
            ...
            qs:
            {
                lat: req.body.lognitude,
                lon: req.body.latidide,
        ...
        await request(options, function async(error, response, body) {
            if (error) throw new Error(error);
            
            
            var body1  = JSON.parse(body);
            var sun =  body1.sys;
            console.log(typeof(sun.sunset));
             var sunset =new Date((sun.sunset+18000)*1000);
             var sunrise =new Date((sun.sunrise+18000)*1000); 
            
             res.json({sunset: sunset, sunrise: sunrise });
        });
